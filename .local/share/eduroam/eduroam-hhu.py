...
class InstallerData(object):
    ...
    def __init__(self, silent=False, username='', password='', pfx_file=''):
        ...
        if Config.tou != '':
            if self.ask(Config.tou, Messages.cont, 1):
                sys.exit(1)
        if os.path.exists(os.environ.get('XDG_DATA_HOME') + '/cat_installer'):
            if self.ask(Messages.cat_dir_exists.format(
                    os.environ.get('XDG_DATA_HOME') + '/cat_installer'),
                        Messages.cont, 1):
                sys.exit(1)
        else:
            os.mkdir(os.environ.get('XDG_DATA_HOME') + '/cat_installer', 0o700)
    ...
    def save_ca(self):
        ...
        """
        certfile = os.environ.get('XDG_DATA_HOME') + '/cat_installer/ca.pem'
        debug("saving cert")
    ...
    def __process_p12(self):
        debug('process_p12')
        pfx_file = os.environ['XDG_DATA_HOME'] + '/cat_installer/user.p12'
        if CRYPTO_AVAILABLE:
            debug("using crypto")
            try:
    ...
    def __save_sb_pfx(self):
        ...
        certfile = os.environ.get('XDG_DATA_HOME') + '/cat_installer/user.p12'
    ...
    def __get_p12_cred(self):
        ...
        if Config.eap_inner == 'SILVERBULLET':
            ...
            if self.silent:
                ...
                try:
                    copyfile(pfx_file, os.environ['XDG_DATA_HOME'] +
...
class WpaConf(object):
    ...
    def __prepare_network_block(self, ssid, user_data):
        ...
        ca_cert=\"""" + os.environ.get('XDG_DATA_HOME') + """/cat_installer/ca.pem\"
    ...
    def create_wpa_conf(self, ssids, user_data):
        ...
        wpa_conf = os.environ.get('XDG_DATA_HOME') + \
...
class CatNMConfigTool(object):
    ...
    def __check_opts(self):
        ...
        """
        self.cacert_file = os.environ['XDG_DATA_HOME'] + '/cat_installer/ca.pem'
        self.pfx_file = os.environ['XDG_DATA_HOME'] + '/cat_installer/user.p12'
...
Config.instname = "Heinrich-Heine-Universität Düsseldorf"
Config.profilename = "HHU Eduroam"
Config.url = " \
    ""http://www.zim.hhu.de/services-des-zim/netz/netzzugang/wireless-lan.html"
Config.email = "wlan@hhu.de"
Config.title = "eduroam CAT"
Config.server_match = "radius.hhu.de"
...
Config.user_realm = "hhu.de"
Config.ssids = ['eduroam']
Config.del_ssids = []
Config.servers = ['DNS:radius.hhu.de']
Config.use_other_tls_id = False
Config.anonymous_identity = "eduroam@hhu.de"
Config.tou = ""
